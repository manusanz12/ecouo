				...
				if($response->status == 200){
					...
					if(preg_match('/^[0-9A-Za-zñÑáéíóúÁÉÍÓÚ ]{1,50}$/', $_POST["name-noticie"] )){
						...
						if($continueEdit){
							...
							$data = "name_noticie=".trim(TemplateController::capitalize($_POST["name-noticie"]))."&url_noticie=".trim($_POST["url-name_noticie"])."&id_category_noticie=".explode("_",$_POST["name-category"])[0]."&image_noticie=".$saveImagenoticie."&description_noticie=".urlencode(trim(TemplateController::htmlClean($_POST["description-noticie"])))."&tags_noticie=".json_encode(explode(",",$_POST["tags-noticie"]))."&gallery_noticie=".json_encode($gallerynoticie)."&video_noticie=".$video_noticie."&top_banner_noticie=".json_encode($topBanner)."&default_banner_noticie=".$saveImageDefaultBanner."&horizontal_slider_noticie=".json_encode($hSlider)."&vertical_slider_noticie=".$saveImageVSlider."&offer_noticie=".$offer_noticie;
