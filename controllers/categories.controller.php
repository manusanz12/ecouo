			...
			if($id == $_POST["idCategory"]){
				...
				if($response->status == 200){
					...
					if(preg_match('/^[A-Za-zñÑáéíóúÁÉÍÓÚ ]{1,}$/', $_POST["name-category"] ) && 
						...
						$data = "name_category=".trim(TemplateController::capitalize($_POST["name-category"]))."&title_list_category=".json_encode(explode(",",$_POST["titleList-category"]))."&url_category=".trim($_POST["url-name_category"])."&image_category=".$saveImageCategory;
