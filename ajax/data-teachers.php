...
class DatatableController{
	...
	public function data(){
		...
		if(!empty($_POST)){
            ...
            $url = "relations?rel=users,roles,campuses&type=user,role,campus&select=id_user,matricula_user,name_user,email_user,id_role_user,id_campus_user,id_role,name_role,id_campus,name_campus&linkTo=date_created_user&between1=".$_GET["between1"]."&between2=".$_GET["between2"]."&filterTo=id_role_user&inTo=2";
            ...
            if(!empty($_POST['search']['value'])){
            	...
            	if(preg_match('/^[0-9A-Za-zñÑáéíóú ]{1,}$/',$_POST['search']['value'])){// Validar que no coloquen caracteres alfanumericos
	            	...
	            	foreach ($linkTo as $key => $value) {
	            		...
	            		$url = "relations?rel=users,roles,campuses&type=user,role,campus&select=".$select."&linkTo=".$value.",id_role_user&search=".$search.",2&orderBy=".$orderBy."&orderMode=".$orderType."&startAt=".$start."&endAt=".$length;
	            ...
	            $url = "relations?rel=users,roles,campuses&type=user,role,campus&select=".$select."&linkTo=date_created_user&between1=".$_GET["between1"]."&between2=".$_GET["between2"]."&filterTo=id_role_user&inTo=2&orderBy=".$orderBy."&orderMode=".$orderType."&startAt=".$start."&endAt=".$length;
